#!/usr/bin/env bash
# setup.sh — Interactive first-run setup wizard for Pembrook
#
...
#       @services — all bridges AND all MCP servers share this one  (optional third atSign)
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo "                      Optional: SERVICES_AT_SIGN, SERVICES_KEYS_PATH,"
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║       Pembrook  —  First-Run Setup           ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo "Pembrook requires at least TWO atSigns:"
...
echo "or MCP servers (Home Assistant, database), you need a THIRD atSign:"
echo "  • @services — all bridges AND all MCP servers share this one atSign"
echo "                (no separate atSign per bridge or per MCP server needed)"
echo ""
echo "Maximum ever needed: 3 atSigns."
...
# Services atSign (optional — used by bridges AND MCP servers)
SERVICES_AT_SIGN=$(prompt_or_env SERVICES_AT_SIGN "Services atSign for bridges/MCP servers (leave blank to skip)" "")
if [[ -n "$SERVICES_AT_SIGN" ]]; then
  [[ "$SERVICES_AT_SIGN" != @* ]] && SERVICES_AT_SIGN="@$SERVICES_AT_SIGN"
  DEFAULT_SERVICES_KEYS="$KEYS_DIR/${SERVICES_AT_SIGN}_key.atKeys"
  SERVICES_KEYS_PATH=$(prompt_or_env SERVICES_KEYS_PATH "Services .atKeys file path" "$DEFAULT_SERVICES_KEYS")
  if [[ ! -f "$SERVICES_KEYS_PATH" ]]; then
    error "Services .atKeys file not found: $SERVICES_KEYS_PATH"
    exit 1
  fi
  success "Services .atKeys found"
else
  SERVICES_KEYS_PATH=""
fi
...
[[ -n "$SERVICES_AT_SIGN" ]] && ALL_ALLOWED="$ALL_ALLOWED,$SERVICES_AT_SIGN"
...
SERVICES_KEY_FILE=""
[[ -n "$SERVICES_KEYS_PATH" ]] && SERVICES_KEY_FILE="$(basename "$SERVICES_KEYS_PATH")"

cat > "$ENV_FILE" <<EOF
# Pembrook environment configuration — generated by scripts/setup.sh
...
$([ -n "$SERVICES_AT_SIGN" ] && echo "SERVICES_AT_SIGN=$SERVICES_AT_SIGN" || echo "# SERVICES_AT_SIGN=")
...
$([ -n "$SERVICES_KEY_FILE" ] && echo "SERVICES_KEY_FILE=$SERVICES_KEY_FILE" || echo "# SERVICES_KEY_FILE=")
...
[[ -n "$SERVICES_KEYS_PATH" ]] && copy_if_needed "$SERVICES_KEYS_PATH"
...
[[ -n "$SERVICES_AT_SIGN" ]] && INIT_ARGS+=("--services-atsign" "$SERVICES_AT_SIGN")
...
[[ -n "$SERVICES_AT_SIGN" ]] && echo "  Services atSign: $SERVICES_AT_SIGN  (bridges + MCP servers)"
...
echo ""
if [[ -n "$SERVICES_AT_SIGN" ]]; then
  echo "  4. Bridges and MCP servers will authenticate as: ${SERVICES_AT_SIGN}"
  echo "     Uncomment the relevant service(s) in docker-compose.yml and add"
  echo "     the required credentials to .env (see GETTING_STARTED.md §10-11)"
  echo ""
fi
