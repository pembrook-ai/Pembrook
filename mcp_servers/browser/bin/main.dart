...
const _userAgent = 'Mozilla/5.0 (compatible; PembrookBot/1.0; +https://github.com/pembrook)';

// ── H1+H2: SSRF protection (redirect-aware, IP-pinned) ─────────────────────

const _blockedHosts = ['localhost', 'host.docker.internal', 'kubernetes.default', 'metadata.google.internal'];
...
void main(List<String> args) async {
  ...
  Logger.root.onRecord.listen((r) => stderr.writeln('[${r.level.name}] ${r.time} ${r.message}'));
  ...
  try {
    ...
    stderr.writeln('Usage: dart run bin/main.dart --atsign @mcp_browser --key-file /path/to/@mcp_browser_key.atKeys');
    exit(1);
  }
  ...
  Logger.root.level = Level.INFO;
  _log.info('Browser MCP server started as ${atClient.getCurrentAtSign()}');
  ...
  try {
    final key =
        (AtKey.shared(
          ...
          )..sharedWith(target)).build()
          ..metadata = (Metadata()
            ..ttl = 30000
            ..ttr = -1);

    await atClient
        .put(key, 'warmup', putRequestOptions: PutRequestOptions()..useRemoteAtServer = true)
        .timeout(const Duration(seconds: 120));

    _log.info(
      'Shared-key warmup with $target succeeded '
      '(${sw.elapsedMilliseconds} ms)',
    );
  } catch (e) {
    _log.warning(
      'Shared-key warmup with $target failed after '
...
Future<void> _listen(AtClient atClient) async {
  ...
  atClient.notificationService.subscribe(regex: _requestPattern, shouldDecrypt: true).listen((notification) async {
    try {
      ...
      if (e is TimeoutException) {
        _notifyFailures++;
        _log.warning('Notify timed out on request ($_notifyFailures/$_maxNotifyFailures)');
  ...
  Timer.periodic(const Duration(minutes: 2), (_) async {
    try {
      ...
      final pingKey = (AtKey.shared('mcp.keepalive', namespace: _namespace, sharedBy: self)..sharedWith(self)).build()
        ..metadata = (Metadata()
          ..ttl = 30000
          ..ttr = -1);
      ...
      _notifyFailures++;
      _log.warning('Keepalive failed ($_notifyFailures/$_maxNotifyFailures): $e');
      if (_notifyFailures >= _maxNotifyFailures) {
        _log.severe('Notify path dead — exiting for Docker restart');
        exit(1);
      }
...
Future<void> _handleNotification(AtClient atClient, AtNotification notification) async {
  ...
  final responseKey =
      (AtKey.shared(
          ...
          namespace: _namespace,
          sharedBy: atClient.getCurrentAtSign() ?? '',
        )..sharedWith(callerAtSign)).build()
        ..metadata = (Metadata()
          ..ttl = 30000
          ..ttr = -1);

  _log.info('Sending response via put() to $callerAtSign for request $requestId ...');
  try {
    await atClient
        .put(responseKey, jsonEncode(response), putRequestOptions: PutRequestOptions()..useRemoteAtServer = true)
...
Future<List<Map<String, dynamic>>> _callTool(String toolName, Map<String, dynamic> args) async {
  switch (toolName) {
    case 'browser.fetch':
...
List<Map<String, dynamic>> _toolList() => [
  ...
  },
  {
    ...
    'description': 'Fetch a URL and return human-readable text (removes HTML boilerplate).',
    'inputSchema': {
      'type': 'object',
      'required': ['url'],
      'properties': {
  ...
  },
  {
    ...
    'description': 'Take a screenshot of a URL (requires Playwright sidecar).',
    'inputSchema': {
      'type': 'object',
      'required': ['url'],
      'properties': {
  ...
  },
  {
    ...
    'description': 'Fill and submit an HTML form (requires Playwright sidecar, HITL required).',
    'inputSchema': {
      'type': 'object',
      'required': ['url', 'fields'],
      'properties': {
...
Future<List<Map<String, dynamic>>> _extractText(Map<String, dynamic> args) async {
  final url = _req(args, 'url') as String;
  final timeoutMs = (args['timeoutMs'] as int?) ?? 15000;
  ...
  return [
    {'type': 'text', 'text': '${title != null ? 'Title: $title\n\n' : ''}$text'},
  ];
}
...
Future<List<Map<String, dynamic>>> _withPlaywright(Map<String, dynamic> args, String action) async {
    ...
    return [
      {
        'type': 'text',
        'text':
            'browser.$action requires a running Playwright sidecar. '
  ...
  final response = await http
      .post(Uri.parse(endpoint), headers: {'Content-Type': 'application/json'}, body: jsonEncode(args))
      .timeout(const Duration(seconds: 60));

  if (response.statusCode != 200) {
    throw StateError('Playwright sidecar returned ${response.statusCode}: ${response.body}');
  }
...
String _cleanText(Document document) {
  ...
  final content = document.querySelector('main') ?? document.querySelector('article') ?? document.querySelector('body');
  ...
  return parts.join().replaceAll(RegExp(r'[ \t]+'), ' ').replaceAll(RegExp(r'\n{3,}'), '\n\n').trim();
}

void _collectText(Element element, List<String> parts) {
  const blockTags = {'p', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'tr', 'br', 'blockquote', 'pre', 'section'};
